...
1. - Сканирование показало наличие двух открытых портов, HTTP(80) и SSH(22) соответственно
2. - Активная разведка веб-сайта показала наличие директорий к которым обычный пользователь не должен был иметь доступ
3. - "Скрытый функционал" (который никак не защищен контролем доступа) сайта позволил верифицировать наличие пользователей в системе, что потенциально облегчает перебор пароля злоумышленнику
4. - Комментарии разработчиков в коде дают явные подсказки
5. - Директория /panel уязвима к SQLI, что позволило спокойно смотреть содержимое базы
6. - Пользователь в самой системе имеет особые права на выполнение скрипта который буквально одним запуском делает этого самого пользователем - владельцем всей системы
